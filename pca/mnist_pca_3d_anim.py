...
mu = np.mean(X,axis=0)
X = X - mu
cov = np.matmul(X.transpose(1,0), X)
...
def update(frame):
    ax.view_init(elev=30, azim=frame)  # Elevation konstant, Azimut rotiert
    return ax,
...
ani = FuncAnimation(
    fig,
    update,
    frames=np.arange(0, 360, 2),  # 0° bis 358°
    interval=50,                  # 50 ms zwischen Frames
    blit=False
)
