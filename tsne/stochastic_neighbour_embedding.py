...
def compute_distance_matrix(data: np.ndarray) -> np.ndarray: # TODO: without any loops!!
    """
    ...
    n = data.shape[0]  # Anzahl Datenpunkte
    XTX = data @ data.T

    distances = np.array([[XTX[i, i] + XTX[j, j] - 2 * XTX[j][i] for i in range(n)] for j in range(n)])
    return distances # done with loops but returns correct result


def compute_high_dim_similarity_matrix(distance_matrix: np.ndarray, sigma_array: np.ndarray) -> np.ndarray: # TODO
    """
...
def compute_gradient_tsne(y: np.ndarray, similarity_matrix_high_dim: np.ndarray,
                          similarity_matrix_low_dim: np.ndarray) -> np.ndarray: # TODO
    """
    ...
    n = similarity_matrix_high_dim.shape[0]

    for i in range(n):
        grad_y[i] = 4 * np.sum([(similarity_matrix_high_dim[i,j] - similarity_matrix_low_dim[i,j])
                                / (1 + (y[i]-y[j]).transpose() @ (y[i]-y[j])) * (y[i] - y[j])
                                for j in range(n)])
...
def train_tsne(data: np.ndarray, num_iterations: int = 500, perplexity: float = 20, exaggeration: float = 4,
               exaggeration_iter_thresh: int = 50) -> np.ndarray: # TODO
    """
    this function incorporates the tsne training loop.

    :param data: 2d numpy array representing the high-dimensional data, where data.shape[0] corresponds
    ...
    :param exaggeration_iter_thresh: iteration beginning with which exxaggeration is omitted
    ...
    for k in range(0, num_iterations):
        ...
        grad = compute_gradient_tsne(y, similarities_high_dim, similarities_low_dim)
...
def main():
    data_root_path = './data'             # path to directory containing 'sign_mnist_train.csv', 'sign_mnist_test.csv'
