...
def compute_distance_matrix(data: np.ndarray) -> np.ndarray:
    ...
    row_sq = np.sum(data * data, axis=1).reshape(-1, 1)
    return row_sq + np.transpose(row_sq) - 2 * np.matmul(data, np.transpose(data))
...
def compute_high_dim_similarity_matrix(distance_matrix: np.ndarray, sigma_array: np.ndarray) -> np.ndarray:
    ...
    """
    tmp = -0.5 * distance_matrix / (sigma_array ** 2).reshape(-1, 1)

    exp_terms = np.exp(tmp - np.max(tmp, axis=1).reshape(-1, 1))
    np.fill_diagonal(exp_terms, 0.0)

    row_sum_array = np.sum(exp_terms, axis=1).reshape(-1, 1)
    return exp_terms / row_sum_array
...
def compute_low_dim_similarity_matrix_ssne(distance_matrix: np.ndarray) -> Optional[np.ndarray]:
    tmp = np.exp(-distance_matrix)
...
def compute_gradient_ssne(y: np.ndarray, similarity_matrix_high_dim: np.ndarray,
                          similarity_matrix_low_dim: np.ndarray, distance_matrix: np.ndarray) -> Optional[np.ndarray]:
    sim_diff = similarity_matrix_high_dim - similarity_matrix_low_dim
    sim_diff = np.expand_dims(sim_diff, 2)
    y_diff = np.expand_dims(y, 1) - np.expand_dims(y, 0)
    grad = 4.0 * np.sum(sim_diff * y_diff, axis=1)
    return grad
...
def compute_gradient_tsne(y: np.ndarray, similarity_matrix_high_dim: np.ndarray,
    ...
    dist_inv = 1.0 / (1.0 + compute_distance_matrix(y))
    dist_inv = np.expand_dims(dist_inv, 2)

    grad = 4.0 * np.sum(sim_diff * y_diff * dist_inv, axis=1)
    return grad
...
def train_tsne(data: np.ndarray, num_iterations: int = 500, perplexity: float = 20, exaggeration: float = 4,
    ...
    for k in range(0, num_iterations):
        distance_matrix_low_dim = compute_distance_matrix(y)
        similarities_low_dim = compute_low_dim_similarity_matrix_tsne(distance_matrix_low_dim)
        similarities_low_dim = np.maximum(similarities_low_dim, 1e-10)

        curr_gradient = compute_gradient_tsne(y, similarities_high_dim, similarities_low_dim)

        y_new = y - alpha * curr_gradient + beta * (y - y_old)
...
def train_ssne(data, num_epochs: int = 500, perplexity: float = 20) -> np.ndarray:
    """
    this function is the main training function.

    :param data: 2d numpy array, where the i-th row contains the i-th data item
    :param num_epochs: maximal number of training epochs
    :param perplexity: floating point number which is used to choose the parameters sigma_i
    :return: 2d numpy array, where the i-th row contains the low-dimensional representative of the i-th data item
    """
    alpha = 10
    beta = 0.9
    ...
    for k in range(0, num_epochs):
        distance_matrix_low_dim = compute_distance_matrix(y)
        similarities_low_dim = compute_low_dim_similarity_matrix_ssne(distance_matrix_low_dim)
        curr_gradient = compute_gradient_ssne(y, similarities_high_dim, similarities_low_dim,
                                              distance_matrix_low_dim)
