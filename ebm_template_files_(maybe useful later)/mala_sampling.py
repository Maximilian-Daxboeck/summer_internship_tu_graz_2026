...
def mala(
    ...
    """
    this function implements the MALA scheme to generate samples from distribution
    with Gibbs-type PDF proportional to density_func, and
    energy gradient function grad_energy_func

    NOTE
        > the initial value x0 is assumed to be of shape (2, num_chains) - if num_chains > 1, the
            algorithm creates num_chains independent chains of MALA iterates.
...
def grad_energy_func_double_banana(
        x1: np.ndarray,
        x2: np.ndarray,
        a: float,
        b: float
...
def density_func_double_banana(
        x1: np.ndarray,
        x2: np.ndarray,
        a: float,
        b: float
    ...
    NOTE:
    ----
        > We do not take into consideration the normalising constant here.
...
def visualise(
    ...
    """
    this function visualises the exact density of the double-banana
    distribution (contour plot) and a sample of it (scatter plot)
...
def main():
    ...
    num_iterations = 'TODO: Fill me'
    num_chains = 'TODO: Fill me'
    gamma = 'TODO: Fill me'
    x0 = 2 * (np.random.rand(2, num_chains) - 1)
    burn_in = 'TODO: Fill me'

    langevin_sample = mala(
        lambda x: grad_energy_func_double_banana(x[0], x[1], a, b),
    ...
    fig = visualise(langevin_sample, a, b)
